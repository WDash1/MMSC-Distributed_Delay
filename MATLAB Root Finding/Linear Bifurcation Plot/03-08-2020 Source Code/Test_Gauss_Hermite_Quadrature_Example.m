...
figure
hold on
plot(solution.x, y_values(1,:));
plot(solution.x, y_values(2,:));
%plot(solution.x, exp(solution.x));
legend("x Approximation", "y Approximation","exp(t)");
xlabel("t");
hold off
...
x_err = abs(y_values(1,:) - exp(solution.x))
y_err = abs(y_values(2,:) - exp(solution.x))

plot(solution.x, abs(y_values(1,:) - exp(solution.x)));
