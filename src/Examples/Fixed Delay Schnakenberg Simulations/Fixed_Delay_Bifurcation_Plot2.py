...
def computeEigenvalues(a,b, tau):
    ...
    rranges = (slice(-2, 2, 1/64), slice(0, 1, 1/32))

    resbrute = optimize.brute(optimisation_function, rranges, full_output=True, finish=optimize.fmin);


    evaluation_values = resbrute[3];
    minimisation_indicies = NP.argwhere(evaluation_values<0.05);
    
    complex_mapper = lambda i: resbrute[2][0][minimisation_indicies[i][0], minimisation_indicies[i][1]] +  resbrute[2][1][minimisation_indicies[i][0], minimisation_indicies[i][1]]*1j;
    
    eigenvalues = list(map(complex_mapper, list(range(0,len(minimisation_indicies)))));
    ...
    #re_initial_guesses = NP.linspace(minimiser[0]-2,minimiser[0]+2,num=10);
    #im_initial_guesses = NP.linspace(minimiser[1]-2,minimiser[1]+2,num=10);
